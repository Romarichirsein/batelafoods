// scripts/upload_product_images.ts
import { createClient } from "@sanity/client";
import * as dotenv from "dotenv";
import * as path from "path";
import * as fs from "fs";
import { productSchema } from "../studio/schemas/product"; // not used directly but ensures type
...
async function main() {
  // Fetch all products ordered by creation date (so order is deterministic)
  ...
  // Simple mapping: assign images sequentially to products (one image per product for now)
  const assignments: { productId: string; files: string[] }[] = [];
  let prodIdx = 0;
  ...
  for (const assign of assignments) {
    ...
    // Patch the product: set images array (append to existing if any)
    const patch = client.patch(productId).setIfMissing({ images: [] }).insert("after", "images[-1]", uploadedRefs);
    try {
      await patch.commit();
